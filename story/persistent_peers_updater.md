...
```bash
sudo mkdir -p $HOME/.peers/
```
#### Create and Edit the Peer Updater Script
```bash
sudo nano $HOME/.peers/peers_checker.sh
```
...
```bash
sudo chmod +x $HOME/.peers/pp_checker.sh
```
...
#### Add Cron Job
```bash
*/5 * * * * ./.peers/pp_checker.sh >> ./.peers/output.log 2>> ./.peers/error.log
```
